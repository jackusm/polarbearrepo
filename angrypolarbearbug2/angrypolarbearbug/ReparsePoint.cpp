...
ScopedHandle OpenReparsePoint(const std::wstring& path, bool writable)
{
	HANDLE h = CreateFile(path.c_str(),		
		GENERIC_READ | (writable ? FILE_WRITE_DATA : 0),
		0,
		0,
		OPEN_EXISTING,
...
bool ReparsePoint::CreateMountPoint(HANDLE hDir, const std::wstring& target, const std::wstring& printname)
{
	ScopedHandle handle(hDir, true);

	return CreateMountPointInternal(handle, BuildMountPoint(FixupPath(target), printname));
}
